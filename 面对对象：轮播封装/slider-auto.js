...
//			var s1 = new Slider(".slider2",{isautoplay:false,startIndex:2});//	创建一个构造器  其类名 为  .slider
		...
		for(var i = 0; i <sliders.length;i++){
			...
			var isautoplay = sliders[i].getAttribute("data-isautoplay");//  获取到   是否 自动播放  的值 
			var istxt = sliders[i].getAttribute("data-istxt");//  获取到   是否   显示文字域  的值 
			var speed = parseInt( sliders[i].getAttribute("data-speed"));//	将 间隔几秒执行一次   转换成 数值
			var startIndex = parseInt( sliders[i].getAttribute("data-startIndex"));//	将 第一次显示第几张   转换成 数值
			
			isautoplay = isautoplay == "false" ?  false  :true;//  判断  是否 自动播放  的值 	 为true 或 false
			istxt = istxt == "false" ?  false  :true;//  判断  是否   显示文字域  的值 	 为true 或 false
